	        ...
	        handle_data : function(dataObj,pageno) {		
		    	...
		    	var readStyle;
 	 			for(var i = 0; i < dataObj.result.length; i ++) {
 	 				readStyle = dataObj.result[i].dealstate == '2' ? 'style="color:#9E9E9E"' : '';
	 			 	rowContent=rowContent 	+ '<li><div class="msg_icon"><img src="${ctx}/static/kingkaid/images/'
	 			 							+ dataObj.result[i].webmsgtypepicture
	 			 							+'"/></div>' 
 											+ '<a id="'+dataObj.result[i].id+'" class="msg-text" '+ readStyle +' onclick="js_method(\''+dataObj.result[i].id+'\',\''+dataObj.result[i].memberid+'\')">'
