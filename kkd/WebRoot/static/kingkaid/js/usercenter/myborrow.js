    ...
    $("#pager").simplePagination({
        ...
        handle_data: function (data) {
            ...
            for (var i = 0; i < dataObj.result.length; i++) {
                ...
                else if (jQuery.inArray(row.apprstate, lookplanstate) > -1)
                    content += '							<div onclick="retuPlan(\'' + row.loanid + '\');" class="manage-loan-icon invest_right_btn">还款计划书</div>';
                else if (row.apprstate === '08')
                    content += '                    <div onclick="showPromptBox(\'' + row.loanid + '\',\'' + row.apprstate + '\')" class="car-loan-icon invest_right_btn">信息确认</div>';
                else if (row.apprstate === '16')
...
//会员信息确认;
function showPromptBox(loanid, apprstate) {
    create_bg();
    var agree_click = ' onclick=\"confirmAjax(\'' + loanid + '\',' + '\'1' + '\',\'' + apprstate + '\');\"';
    var refuse_click = ' onclick=\"confirmApp(\'' + loanid + '\',' + '\'24' + '\');\"';
    $.ajax({
        type: "POST",
        url: ctx + '/auth/usercenter/getBorrowDetailInfo',
        data: {
            loanid: loanid
        },
        dataType: "json",
        success: function (data) {
            if (data.regisarea == '') {
                alert("您还未完善户口所在地信息，请先前往安全中心个人信息栏完善您的户口所在地！");
                location.href = ctx + "/auth/usercenter/safetycenter.html";
            } else {
                if (data.addr == '') {
                    alert("您还未完善住宅地址信息，请先前往安全中心个人信息栏完善您的住宅地址！");
                    location.href = ctx + "/auth/usercenter/safetycenter.html";
                } else {
                    var visual = document.createElement("div");
                    visual.id = "new_dialogue";
                    var prompthtml = '<div class="alert_box_small" style="width:700px;top:8px;">';
                    ...
                    prompthtml += '                <ul>';
                    prompthtml += '                 <li>';
                    prompthtml += '                  <div style="font-size:16px;text-align:center;">申请人基本情况</div>';
                    prompthtml += '                  <div style="width:330px;float:left;height:450px;">';
                    prompthtml += '                    <div>';
                    prompthtml += '                     <span>姓名：' + data.custname + '</span>';
                    prompthtml += '                   </div>';
                    prompthtml += '                   <div>';
                    prompthtml += '                     <span>证件类型：身份证</span>';
                    prompthtml += '                   </div>';
                    prompthtml += '                   <div>';
                    prompthtml += '                     <span>出生年月日：' + data.birtdate + '</span>';
                    prompthtml += '                   </div>';
                    prompthtml += '                   <div>';
                    prompthtml += '                     <span>入学年份：' + data.enterschoolyear + '</span>';
                    prompthtml += '                   </div>';
                    prompthtml += '                   <div>';
                    prompthtml += '                     <span>是否结婚：' + data.marrsignname + '</span>';
                    prompthtml += '                   </div>';
                    prompthtml += '                   <div>';
                    prompthtml += '                     <span>有无房产：' + data.varchar_11 + '</span>';
                    prompthtml += '                   </div>';
                    prompthtml += '                   <div>';
                    prompthtml += '                     <span>有无车辆：' + data.varchar_13 + '</span>';
                    prompthtml += '                   </div>';
                    prompthtml += '                   <div>';
                    prompthtml += '                     <span>车辆品牌：' + data.carbrand + '</span>';
                    prompthtml += '                   </div>';
                    prompthtml += '                   <div>';
                    prompthtml += '                     <span>户口所在地：' + data.regisarea + '</span>';
                    prompthtml += '                   </div>';
                    prompthtml += '                   <div>';
                    prompthtml += '                     <span>住宅地址：' + data.addr + '</span>';
                    prompthtml += '                   </div>';
                    prompthtml += '                   <div>';
                    prompthtml += '                     <span>居住地电话：' + data.tel + '</span>';
                    prompthtml += '                   </div>';
                    prompthtml += '                   <div>';
                    prompthtml += '                     <span>入职年份：' + data.workyearmonth + '</span>';
                    prompthtml += '                   </div>';
                    prompthtml += '                   <div>';
                    prompthtml += '                     <span>公司行业：' + data.waykindname + '</span>';
                    prompthtml += '                   </div>';
                    prompthtml += '                   <div>';
                    prompthtml += '                     <span>公司地址：' + data.corpaddr + '</span>';
                    prompthtml += '                   </div>';
                    prompthtml += '                  </div>';

                    prompthtml += '                  <div style="width:270px;float:left;height:450px;">';
                    prompthtml += '                    <div>';
                    prompthtml += '                     <span>性别：' + data.sexsign + '</span>';
                    prompthtml += '                   </div>';
                    prompthtml += '                   <div>';
                    prompthtml += '                     <span>证件号码：' + data.paperid + '</span>';
                    prompthtml += '                   </div>';
                    prompthtml += '                   <div>';
                    prompthtml += '                     <span>最高学历：' + data.educsignname + '</span>';
                    prompthtml += '                   </div>';
                    prompthtml += '                   <div>';
                    prompthtml += '                     <span>毕业院校：' + data.gradschool + '</span>';
                    prompthtml += '                   </div>';
                    prompthtml += '                   <div>';
                    if (data.childsignname == '0') {
                        prompthtml += '                     <span>有无子女：否</span>';
                    } else if (data.childsignname == '1') {
                        prompthtml += '                     <span>有无子女：是</span>';
                    } else {
                        prompthtml += '                     <span>有无子女：</span>';
                    }
                    prompthtml += '                   </div>';
                    prompthtml += '                   <div>';
                    prompthtml += '                     <span>有无房贷：' + data.varchar_12 + '</span>';
                    prompthtml += '                   </div>';
                    prompthtml += '                   <div>';
                    prompthtml += '                     <span>有无车贷：' + data.varchar_14 + '</span>';
                    prompthtml += '                   </div>';
                    prompthtml += '                   <div>';
                    prompthtml += '                     <span>购车年份：' + data.carbuyyear + '</span>';
                    prompthtml += '                   </div>';
                    prompthtml += '                   <div>';
                    prompthtml += '                     <span>籍贯：' + data.nativeplace + '</span>';
                    prompthtml += '                   </div>';
                    prompthtml += '                   <div>';
                    prompthtml += '                     <span>移动电话：' + data.mobilenumber + '</span>';
                    prompthtml += '                   </div>';
                    prompthtml += '                   <div>';
                    prompthtml += '                     <span>工作单位：' + data.workcorp + '</span>';
                    prompthtml += '                   </div>';
                    prompthtml += '                   <div>';
                    prompthtml += '                     <span>职业：' + data.vocasignname + '</span>';
                    prompthtml += '                   </div>';
                    prompthtml += '                   <div>';
                    prompthtml += '                     <span>单位电话：' + data.corptel + '</span>';
                    prompthtml += '                   </div>';
                    prompthtml += '                  </div>';
                    prompthtml += '                  </li>';
                    prompthtml += '                   <li style="font-size:16px;text-align:center;">项目基本信息</li>';
                    prompthtml += '                  <div style="width:330px;float:left;">';
                    prompthtml += '                   <div>';
                    ...
                    prompthtml += '                   </div>';
                    prompthtml += '                   </div>';
                    prompthtml += '                  <div style="width:270px;float:left;">';
                    prompthtml += '                   <div>';
                    ...
                    if (data.prodid != '100201' && data.prodid != '100301') {
                        ...
                        prompthtml += '                   <div>借款项目还款来源:';
                        prompthtml += '                     <span>' + data.repafundsour + '</span>';
                        prompthtml += '                   </div>';
                        prompthtml += '                   <div>征信情况:';
                        prompthtml += '                     <span>' + data.creditstate + '</span>';
                        prompthtml += '                   </div>';

                        if (data.prodid != '100401') {
                            prompthtml += '                   <div>担保机构:';
                            prompthtml += '                     <span>' + data.dbcustname + '</span>';
                            prompthtml += '                   </div>';
                        }
                    }
                    ...
                    prompthtml += '                   </div>';
                    prompthtml += '                   <li style="text-align:center;font-size:16px;">申请人声明</li>';
                    prompthtml += '                   <div>本人声明：</div>';
                    prompthtml += '                   <div>';
                    prompthtml += '                     <span>1、贵机构经办项目经理已就上述情况与本人进行了面谈。</span>';
                    prompthtml += '                   </div>';
                    prompthtml += '                   <div>';
                    prompthtml += '                     <span>2、申请表中填写内容、资料均属真实，如有隐瞒或虚构，本人将承担一切法律和经济后果。本人同时授权贵机构向有关方面查询、核实相关情况。</span>';
                    prompthtml += '                   </div>';
                    prompthtml += '                   <div>';
                    prompthtml += '                     <span>3、本人授权贵机构通过中国人民银行个人信息基础数据库等依法成立的个人信用数据库查询本人信用报告，并将本人的身份识别、职业和居住地址等个人基本信息，本人个人信贷交易记录，以及其他相关信用信息报送中国人民银行征信中心。</span>';
                    prompthtml += '                   </div>';
                    prompthtml += '                </ul>';
                    prompthtml += '             </div>';
                    prompthtml += '       </div>';
