...
(function($) {
	...
	var intr,
		...
		options = {
			...
			secDuration: 0,                             // seconds as duration to run callback after
	...
	function makeEditable() {
		...
		$el.on('blur', function() {
			...
			} else if (val.match(/\d{1,2} \d{1,2}:\d{2}:\d{2}/)) {
				// days
				var day = val.split(' ')[0];
				valArr = val.split(' ')[1].split(':');
				totalSeconds = Number(day * 86400) + (valArr[0] * 3600) + Number(valArr[1] * 60) + Number(valArr[2]);
			}
			resumeTimer();
		});
	}
	...
	function sec2TimeObj(sec) {
		var days = 0, hours = Math.floor(sec / 3600), minutes = Math.floor(sec / 60), seconds;
	...
	function secondsToTime(sec) {
		...
		if (options.format) {
			var formatDef = [
				{identifier: '%d', value: timeObj.days, pad: false},
				...
				{identifier: '%D', value: parseInt(timeObj.days), pad: true},
			...
			for(var i = 0; i < formatDef.length; i ++) {
				time = time.replace(
					new RegExp(formatDef[i].identifier.replace(/([.*+?^=!:${}()|\[\]\/\\])/g, '\\$1'), 'g'),
					(formatDef[i].pad) ? ((formatDef[i].value < 10) ? '0' + formatDef[i].value : formatDef[i].value) : formatDef[i].value
				);
			}

		} else {
			if (timeObj.days) {
				time = timeObj.days + ' ' + timeObj.hours + ':' + timeObj.minutes + ':' + timeObj.seconds;
	...
	function timeToSeconds(time) {
		...
		var dMatch = time.match(/\d{1,2}d/),
			hMatch = time.match(/\d{1,2}h/),
        ...
        if (dMatch) {
			seconds += Number(dMatch[0].replace('d', '')) * 86400;
		}
